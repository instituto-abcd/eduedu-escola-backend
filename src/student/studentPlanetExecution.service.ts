...
export class StudentPlanetExecutionService {
	...
	private verifyAnswerPlanet_MODEL11(
		...
		const expectedOptionAnswers = question.rules
			.find((rule) => rule.name === "answers")
			?.value.replace('"', "")
			.replace('"', "");
	...
	private verifyAnswerPlanet_MODEL12(
		...
		const allAnswersAreCorrect = answerOptions.every(
			(answeredOption) =>
	...
	private verifyAnswerPlanet_MODEL19(
		...
		const rule = question.rules.find((item) => item.name == "verify").value;

		const targets = question.titles
			.find((item) => item.type == "TEXT" && item.description.includes(" "))
			.description.split(" ");

		const answers = answerOptions.map(
			(answerOption) => answerOption.description,
		);
		let asserts = [];
		...
		for (let index = 0; index < answers.length; index++) {
			...
			switch (rule) {
				case "starts_with":
					asserts.push(
						answer.toLowerCase().startsWith(targets[index].toLowerCase()),
					);
					break;
				case "ends_with":
					asserts.push(
						answer.toLowerCase().endsWith(targets[index].toLowerCase()),
					);
					break;
				case "contains":
					asserts.push(
						answer.toLowerCase().includes(targets[index].toLowerCase()),
					);
					break;
			}
		}
